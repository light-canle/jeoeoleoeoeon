...
- 난해한 프로그래밍 언어를 만드는 HeXA 동아리 프로젝트에서 '저런'이라는 단어를 모티브로 한 난해한 프로그래밍 언어를 제작할 것을 계획함
- 컴퓨터구조 과목에서 배우는 어셈블리 명령어처럼 1줄 단위의 명령어 구조를 가진 언어를 만들어 보면 어떨까라는 생각에 '저런' 단어를 길게 늘린 '저어러어언' 형태를 1줄로 하고, 숫자 대신에 저, 러, 언 사이에 '어' 나 '.'를 넣어 명령어 번호, 영향을 받는 변수 번호를 입력하는 형식으로 제작을 했다.
...
> - 명령어 번호에 s가 앞에 붙는다.
> - s24, s27번 명령어는 one 타입 명령어이며 rs, rd, imm 모두 없이 아래 처럼 op만 써준다.
...
| 명령어 코드| 이름     |   설명 | 타입 |
| ----------|--------| --------------------------------         | ----- |
| 10       | mul3x1 | val[rs] = val[rs] * 3 + 1를 imm번 반복한다. | semi-full |
| 11       | div2   | val[rs] = val[rs] / 2 를 imm번 반복한다.(소수점 버림)  | semi-full |
...
| 46        | not    | rs번 변수 값에 비트 not 연산을 수행한 뒤 그 변수에 저장한다.  | half |
...
| 74        | j      | 조건없이 rs번의 레이블으로 점프한다.  | half |
...
| s22        | pright  | 포인터의 위치를 오른쪽으로 imm[3x+1]칸 만큼 이동시킨다.  |  semi-full |
| s23        | prighti | 포인터의 위치를 오른쪽으로 imm칸 만큼 이동시킨다.  |  semi-full |
| s24        | pset0   | 포인터가 0번 변수를 가르키게 한다.  |  one |
| s25        | pputv   | 포인터가 가르키는 변수에 rs번 변수의 값을 대입한다. | half |
| s26        | pputi   | 포인터가 가르키는 변수에 imm[3x+1]을 대입한다. | semi-full |
| s27        | pput0   | 포인터가 가르키는 변수에 0을 대입한다. | one |
| s28        | pget    | rs번 변수에 포인터가 가르키는 변수의 값을 대입한다. | half |
...
- '저', '앗', '아' 로 시작하지 않은 모든 문장은 주석 취급받는다.
...
- 2023-12-21 일부 오타 수정, 인터프리터 구현 시작
- 2023-12-22 인터프리터 1차 완성, 유틸리티 기능 완성, 조건없는 분기 명령어 추가, 개정된 문법에 따라 예제 다시 작성
